...
<div class="w-1/2 mx-auto mt-5">
    ...
    <form action="{{ route('student.update', $Student->id) }}" method="POST" class="max-w-md mx-auto">
        ...
        <div class="mb-4">
            ...
            @error('firstname')
                <span class="text-red-500 text-sm" id="error-message-firstname">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            @error('lastname')
                <span class="text-red-500 text-sm" id="error-message-lastname">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            @error('email')
                <span class="text-red-500 text-sm" id="error-message-email">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            @error('phone')
                <span class="text-red-500 text-sm" id="error-message-phone">{{ $message }}</span>
            @enderror
        </div>


        <div class="mb-4">
            <button type="submit" class="bg-indigo-800 text-white py-2 px-4 rounded hover:bg-indigo-600 w-full" id="btn-update">Update</button>
        </div>
        <div>
            <a href="{{ url('/students') }}" class="text-blue-500" id="btn-back">Back</a>
        </div>
    </form>
</div>
@endsection
